...
if __name__ == '__main__':
    ...
    b = Block(num_attention_heads=config.NUM_ATTENTION_HEADS, hidden_size=config.HIDDEN_SIZE,
             linear_dim=config.LINEAR_DIM, dropout_rate=config.DROPOUT_RATE,
             attention_dropout_rate=config.ATTENTION_DROPOUT_RATE, 
             eps=config.EPS, std_norm=config.STD_NORM)
    x = embeddings(x)
    x = b(x)
    print(x)